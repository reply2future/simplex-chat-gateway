...
RUN git clone -b deploy https://github.com/reply2future/simplex-chat.git && mv simplex-chat /project
WORKDIR /project
...
FROM ubuntu:${TAG} AS production
COPY --from=build /project/simplex-chat /usr/local/bin
